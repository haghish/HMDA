...
#' @param domains list of two character vectors, each including a number of features.
#' @param n integer, number of permutations
#' @importFrom shapley shapley.feature.test shapley.domain.test
#' @author E. F. Haghish
...
hmda.test <- function(wmshap, features = NULL, domains = NULL, n = 5000) {

  if (!is.null(features) & !is.null(domains))
    stop("either specify a vector of 'features' or a list of 'domains'")
  else if (is.null(features) & is.null(domains))
    stop("either specify a vector of 'features' or a list of 'domains'")

  else if (!is.null(features)) {
    shapley.feature.test(shapley = wmshap, features = features, n = n)
  }
  else if (!is.null(domains)) {
    shapley.domain.test(shapley = wmshap, domains, n = n)
  }
}
